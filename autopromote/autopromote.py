...
CONFIG_FILE = os.getenv("CONFIG_FILE", "autopromote/autopromote.json")
PKGINFOS_PATHS = []
...
def load_config():
    ...
    # Replace $GITHUB_WORKSPACE with the actual environment variable value
    github_workspace = os.getenv("GITHUB_WORKSPACE")
    if (github_workspace):
        config["munki_repo"] = config["munki_repo"].replace("$GITHUB_WORKSPACE", github_workspace)
...
def output_results(promotions, error):
    ...
    file_path = CONFIG.get("output_results_path", "autopromote/results.plist")
